...
:?:/cpu::
AddChatMessage("{006EE6} CP {FFFFFF} Überweisung vom eigenen Konto an Spieler")
Sleep, 250
...
:?:/scpu::
AddChatMessage("{006EE6} CP {FFFFFF} Überweisung vom eigenen Konto an Spieler (ohne Grund)")
Sleep, 250
recipient := PlayerInput("Empfänger: ")
amount := PlayerInput("Betrag: ")
...
{
AddChatMessage("{006EE6} [CP]: {FFFFFF}Dein "color ""prm[2] "{FFFFFF} läuft nicht ab.")
Return
...
ShowDialog(5,"Teamliste", diatxt, "Schließen")
Return
...
ShowDialog(5,"Leaderliste", diatxt, "Schließen")
Return
...
If (StrLen(diatxt) > 4096)
{
    AddChatMessage("{B22222} FEHLER: {FFFFFF} Der Geldlog ist zu groß zum Anzeigen")
}
Else
{
...
{
    AddChatMessage("Diese Funktion steht dir leider nicht zur Verfügung!")
    Return
}
